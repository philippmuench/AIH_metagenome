...
familyData.raw  <- aggTax(otu.biom, lvl = "X5", out = "MRexperiment", log=TRUE)
mat.familyData.raw  <- aggTax(otu.biom, lvl = "X5", out = "matrix", log=TRUE)
...
p1 <- p1 + geom_boxplot(width = 0.65,  outlier.size=1) + coord_flip() + theme_classic()
#p1 <- p1 + scale_y_log10()
...
grid.newpage()
grid.draw(g)
ggsave(filename = "results/figure_s2.pdf")
