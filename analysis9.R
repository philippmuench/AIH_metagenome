...
familyData.raw  <- aggTax(otu.biom, lvl = "X6", out = "MRexperiment", log=FALSE, norm=T)
mat.familyData.raw  <- aggTax(otu.biom, lvl = "X6", out = "matrix", log=FALSE, norm=T)
...
df_sd <- df_sd[which(rownames(df) != "g__"),]
df <- df[which(rownames(df) != "g__"),]
