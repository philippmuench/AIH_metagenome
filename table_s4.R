...
write.table(fit.top, file="results/table_s4_genus_aih_vs_helathy.tsv", sep="\t", quote=F)
...
fit.top <- topTable(fit2, number=Inf, adjust.method="fdr")
fit.top$star <- add.significance.stars(fit.top$adj.P.Val)
write.table(fit.top, file="results/figure3/table_s4_genus_aih_vs_control.tsv", sep="\t", quote=F)
...
fit.top <- topTable(fit2, number=Inf, adjust.method="fdr")
fit.top$star <- add.significance.stars(fit.top$adj.P.Val)
write.table(fit.top, file="results/table_s4_genus_healthy_vs_control.tsv", sep="\t", quote=F)
