...
familyData.raw  <- aggTax(otu.biom, lvl = "X3", out = "MRexperiment", log=TRUE)
mat.familyData.raw  <- aggTax(otu.biom, lvl = "X3", out = "matrix", log=TRUE)
...
dfx <- df2
dfx$type <- NULL
dfx2 <- dfx[, which(colSums(dfx) > 2.9)]
dfx2_b <- dfx[, which(colSums(dfx) < 2.9)]
...
p1 <- p1 + geom_boxplot(width = 0.24,  outlier.size=1) + coord_flip() + theme_classic()
#p1 <- p1 + scale_y_log10()
...
ggsave(file="results/figure_1a_class.pdf",g)
